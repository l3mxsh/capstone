...
// Fetch invoice — strictly scoped to this client
$stmt = $pdo->prepare("
    SELECT i.*,
           u.name AS client_name, u.email AS client_email,
           p.name AS package, p.price AS package_price,
           p.inclusions,
...
<head>
    ...
    <style>
        ...
        .inv-header .studio-name {
            color: var(--gold);
            font-size: 1.4rem;
            font-weight: 800;
            letter-spacing: .5px;
        }
        .inv-header .studio-meta {
            color: #adb5bd;
            font-size: .8rem;
            margin-top: .3rem;
            line-height: 1.7;
        }
        .inv-header .invoice-label {
            text-align: right;
        }
        .inv-header .invoice-label .inv-no {
            color: var(--gold);
            font-size: 1.5rem;
            font-weight: 800;
        }
        .inv-header .invoice-label .inv-date {
            color: #adb5bd;
            font-size: .8rem;
            margin-top: .2rem;
        }
        ...
        .status-stamp {
            ...
            border: 3px solid <?= $statusColor[$inv['status']] ?? '#6c757d' ?>;
            color: <?= $statusColor[$inv['status']] ?? '#6c757d' ?>;
            font-size: 1.1rem;
            font-weight: 800;
            text-transform: uppercase;
            letter-spacing: 3px;
...
<div class="toolbar no-print">
    ...
    <button class="btn btn-sm text-white" style="background:#1a1a2e;border:none;"
...
<div class="invoice-wrap">
    ...
    <div class="inv-header">
        ...
        <div class="invoice-label">
            ...
            <div class="inv-date">
                Issued: <?= htmlspecialchars($inv['issued_date']) ?>
            </div>
            <div class="mt-2">
                <span class="status-stamp"><?= strtoupper($inv['status']) ?></span>
            </div>
        </div>
    </div>
        ...
        <table class="items-table">
            ...
            <tbody>
                <tr>
                    ...
                    <td style="color:#6c757d;font-size:.82rem;white-space:pre-line;">
                        <?= htmlspecialchars($inv['inclusions'] ?? '—') ?>
                    </td>
                    <td style="text-align:right;font-weight:600;">
                        ₱<?= number_format((float)$inv['package_price'], 2) ?>
                    </td>
                </tr>
            </tbody>
        </table>
