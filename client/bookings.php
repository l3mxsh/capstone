...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'cancel' && $bookingId) {
        ...
        if ($stmt->fetch()) {
            $pdo->prepare("UPDATE bookings SET status='cancelled' WHERE id=? AND client_id=?")->execute([$bookingId,$clientId]);
            $pdo->prepare("DELETE FROM staff_schedules WHERE booking_id=?")->execute([$bookingId]);
            $pdo->prepare("INSERT INTO cancellations (booking_id,client_id,reason,deposit_amount,deposit_retained) VALUES (?,?,?,0,0)")->execute([$bookingId,$clientId,$reason]);
...
$stmt->execute([$clientId]); $bookings = $stmt->fetchAll();
...
$ppStmt->execute([$clientId]); $ppMap = [];
foreach ($ppStmt->fetchAll() as $row) { $ppMap[$row['booking_id']] = $row; }
                    ...
                    <?php else: ?>
                        <?php foreach ($bookings as $b):
                            ...
                            $bJson = htmlspecialchars(json_encode([
                                'id'=>$b['id'],'package'=>$b['package'],'price'=>number_format((float)$b['price'],2),
                                'booking_date'=>$b['booking_date'],'event_type'=>$b['event_type']??'','venue'=>$b['venue']??'',
                                'notes'=>$b['notes']??'','status'=>$b['status'],'created_at'=>date('M d, Y',strtotime($b['created_at'])),
                                'pp'=>$pp ? ['photo_status'=>$pp['photo_status'],'video_status'=>$pp['video_status'],'other_status'=>$pp['other_status'],
                                    'progress_percent'=>$pp['progress_percent'],'deadline'=>$pp['deadline']??'',
                                    'deadline_status'=>$pp['deadline_status']??'early','pp_notes'=>$pp['pp_notes']??'','drive_link'=>$pp['drive_link']??''] : null
                            ]),ENT_QUOTES);
                        ?>
                        <tr>
                            ...
                            <td data-label="Actions">
                                <div class="d-flex gap-1">
                                    ...
                                    <button class="btn-action danger" title="Cancel" onclick="openCancelModal(<?= (int)$b['id'] ?>)">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="info-box mb-3" style="border-color:#fde8e8;background:#fef9f9;">
                        ...
                        <span style="font-size:.82rem;color:#555;">Cancelling an approved booking may result in deposit forfeiture. The admin will process the final decision.</span>
                    </div>
    ...
    function viewDetails(b) {
        ...
        if (b.pp) {
            ...
            document.getElementById('ppDeliverables').innerHTML =
                [['photo_status','Photo'],['video_status','Video'],['other_status','Other']].map(([k,l]) => {
            ...
            } else { document.getElementById('ppNotesWrap').style.display = 'none'; }
        } else {
