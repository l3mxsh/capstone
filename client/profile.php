    ...
    if ($action === 'update_profile') {
        ...
        if (!password_verify($current, $hash)) {
            ...
            $pdo->prepare("UPDATE users SET password=? WHERE id=?")
                ->execute([password_hash($new, PASSWORD_DEFAULT), $clientId]);
        ...
        if (!$file || $file['error'] !== UPLOAD_ERR_OK) {
            ...
            if (!in_array($mime, $allowed)) {
                ...
                // Delete old avatar
                $old = $pdo->query("SELECT avatar FROM users WHERE id=$clientId")->fetchColumn();
                if ($old && file_exists('../assets/avatars/' . $old)) unlink('../assets/avatars/' . $old);
                move_uploaded_file($file['tmp_name'], $dest);
                $pdo->prepare("UPDATE users SET avatar=? WHERE id=?")->execute([$filename, $clientId]);
        ...
        <div id="client-topbar">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    ...
                    <div class="topbar-sub">Manage your account information</div>
                </div>
            </div>
